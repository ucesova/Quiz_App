...
var xhr2; // define the global variable to process the AJAX request
function callDivChange() {
	xhr2 = new XMLHttpRequest();
	var filename = document.getElementById("filename").value;
	xhr2.open("GET", filename, true);
	xhr2.onreadystatechange = processDivChange;
	try {
...
function processDivChange() {
	if (xhr2.readyState < 4) // while waiting response from server
		document.getElementById('ajaxtest').innerHTML = "Loading...";
	else if (xhr2.readyState === 4) { // 4 = Response from server has been completely loaded.
		if (xhr2.status == 200 && xhr2.status < 300)// http status between 200 to 299 are all successful
			document.getElementById('ajaxtest').innerHTML = xhr2.responseText;
	}
}
